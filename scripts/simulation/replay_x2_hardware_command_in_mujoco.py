...
if rclpy is not None:
    ...
    COMMAND_SUB_QOS = None


class X2CommandMirrorNode(Node):
    def __init__(
        self,
        arm_command_topic: str,
        head_command_topic: str,
        tracked_joints: Iterable[str],
    ):
        super().__init__("x2_command_mirror")
    ...
    def _update_from_msg(self, msg) -> int:
        ...
        for joint in self._iter_joint_entries(msg):
            ...
            if joint_name not in self.tracked_joints:
                continue
            self.command_positions[joint_name] = float(getattr(joint, "position", 0.0))
            count += 1
        if count > 0:
...
def main(
    ...
    """Mirror X2 hardware command topics into MuJoCo."""
    ...
    print("Starting X2 hardware-command mirror in MuJoCo...")
    print(f"  arm_command_topic: {arm_command_topic}")
    print(f"  head_command_topic: {head_command_topic}")
    ...
    try:
        with mj_viewer.launch_passive(mj_model, mj_data) as viewer:
            ...
            while True:
                ...
                for joint_name, joint_value in node.command_positions.items():
                ...
                if print_hz > 0.0 and (now - last_print_time) >= (1.0 / print_hz):
                    last_print_time = now
                    ordered = ", ".join(
                        f"{joint_name}={node.command_positions[joint_name]:+.3f}"
                        for joint_name in tracked_joints
                        if joint_name in node.command_positions
                    )
                    print(
                        "Mirror status: "
                        f"arm_msgs={node.arm_msg_count}, head_msgs={node.head_msg_count}, "
                        f"tracked={len(node.command_positions)}, commands=[{ordered}]"
                    )
                ...
                if is_stale and not stale_warned:
                    print(
                        "Warning: command topics look stale. "
                        f"No fresh command seen for {now - node.last_update_time:.2f}s."
                    )
                    stale_warned = True
                elif not is_stale:
                    stale_warned = False
